...
Si marcamos la opción de "*auto redirect*", nos manda a una página web con el protocolo **HTTPS**.

![picture](/assets/images/hackthebox/bus8.png)

Pero probando a introducir "``{2*2}``", no lo resuelve, sino que lo sigue mostrando tal y como he introducido el "**payload**" pero en codificación URL.
...
Investigando un poco más, encontramos la siguiente [**Web**](https://security.snyk.io/vuln/SNYK-PYTHON-SEARCHOR-3166303), que esta versión es vulnerable debido a la implementación insegura del método `eval()`. 
